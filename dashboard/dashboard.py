...
def create_payment_type_df(df):
    payment_type_df = df.groupby('payment_type')['order_id'].nunique()
    return payment_type_df 

def create_review_df(df):
    review_df = df.groupby('review_score')['order_id'].nunique()
    return review_df
...
ax[0].set_title("Best Performing Category Product", loc="center", fontsize=50)
...
ax[1].set_title("Worst Performing Category Product", loc="center", fontsize=50)
...
payment_type_df.plot(kind='pie', autopct='%1.1f%%', startangle=30, colors = ['#ff9999', '#66b3ff', '#99ff99', '#ffcc99', '#c2c2f0', '#ffb3e6', '#c4e17f']
)
plt.title('')
...
st.subheader("Order Distribution by Review Scores")
fig, ax = plt.subplots(figsize=(10, 10))
review_df.plot(kind='bar', color='skyblue')
...
for i, v in enumerate(review_df):
    ax.text(i, v + 0.3, str(v), ha='center', fontsize=12, weight='bold')    

# Menampilkan chart
plt.xticks(rotation=0)  # Memastikan label x-axis horizontal
plt.show()
st.pyplot(fig)


st.subheader("Order Distribution by Delivery Delays")
# Menentukan warna untuk masing-masing nilai True dan False
colors = ['skyblue' if x == False else 'red' for x in late_delivery_df.index]

# Membuat bar chart
fig, ax = plt.subplots(figsize=(10, 10))
ax = late_delivery_df.plot(kind='bar', color=colors)

# Menambahkan judul dan label
plt.title('', fontsize=14, weight='bold')
plt.xlabel('Late in Delivery', fontsize=12)
plt.ylabel('')  # Menghapus label y-axis

# Menambahkan angka di atas setiap bar
for i, v in enumerate(late_delivery_df):
    ax.text(i, v + 0.3, str(v), ha='center', fontsize=12, weight='bold')
...
st.caption('Copyright (c) Ilham Praja 2025')